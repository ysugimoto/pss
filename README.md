...
「$変数名: 値」または「$変数名 = 値」という構文で変数が定義できます。変数の値には、int/string/array/hashが利用できます。
値はシングルクオート、ダブルクオートで括ってもOKです。

**「$変数名: 値」はCSSプロパティの形式に似ていて紛らわしいので、「$変数名 = 値」の形式にする予定です。一応現在は両方サポートしていますが、いずれは未サポートに知ると思うのでご注意下さい。**
